...
ggplot(ex2_results$x_det)
ggplot(ex2_results$x_noise)
ggplot(ex2_results$x_qoi)
```
...
```{r, fig.keep='all', cache=TRUE, eval=LOCAL}
ggplot(ex3_results$x_single)
ggplot(ex3_results$x_qoi)
```
...
```{r, fig.keep='all', cache=TRUE, eval=LOCAL}
ggplot(ex4_results$x_single)
ggplot(ex4_results$x_qoi)
```
...
```{r, cache=TRUE, eval=LOCAL}
ex5_results <- sobol_example_process()
ex5_results
```

```{r, fig.keep='all', cache=TRUE, eval=LOCAL}
ggplot(ex5_results$xp_single)
ggplot(ex5_results$xp_qoi)
```
