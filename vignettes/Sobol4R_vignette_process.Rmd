...
```{r process-indiv-demo}
sobol4r_process_indiv(
  X_indiv = c(
    lambda1 = 1 / 60,
    lambda2 = 1 / 1012,
    lambda3 = 1 / 72,
...
```{r process-sobol-single}
sob_proc_single <- sobol4r_run(
  model = sobol4r_process,
  X1    = X1,
  X2    = X2,
  order = 1,
  nboot = 20,
...
```{r process-sobol-single-plot}
print(sob_proc_single)
autoplot(sob_proc_single)
```
...
```{r process-sobol-qoi-plot}
print(sob_proc_qoi)
autoplot(sob_proc_qoi)
```
...
stochastic. The same workflow can be applied to more complex simulators.
