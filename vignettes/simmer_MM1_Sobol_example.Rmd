...
# Simulation model
```{r model-once}
# simmer_MM1_sobol4r_systemtime.R
...
# Quick sanity check
simulate_mm1_once(lambda = 1/5, mu = 1/4)
```
...
If `ggplot2` and `simmer.plot` are installed, we can visualise the indices with
the autoplot method provided by `sensitivity`.

```{r sobol-plot, message=FALSE, warning=FALSE}
library(ggplot2)
# autoplot(sob_mm1)
```
...
n <- 200
X1 <- data.frame(
  lambda = runif(n, 0.10, 0.30),
  mu     = runif(n, 0.20, 0.60)
)
X2 <- data.frame(
  lambda = runif(n, 0.10, 0.30),
  mu     = runif(n, 0.20, 0.60)
...
Y <- sobol4r_mm1_model(sob_mm1$X, horizon = 1000, warmup = 200, nrep = 20)
sob_mm1 <- tell(sob_mm1, Y)
print(sob_mm1)
```
...
Yc <- sobol4r_clinic_model(sob_clinic$X,
                           cap_reg = 2, cap_exam = 3,
                           horizon = 2000, warmup_prob = 0.2,
                           nrep = 10)
